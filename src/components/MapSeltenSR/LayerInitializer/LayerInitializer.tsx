...
const LayerInitializer = () => {
	const config = useMapStore((state) => state.configSeltenSR);
	const map = useMapStore((state) => state.mapSeltenSR);
	const setLayersInStore = useMapStore((state) => state.setLayersSeltenSR);
	...
	useEffect(() => {
		...
		allManagedLayers.push(
			...subjectLayers.filter(
				(subjectLayer) => subjectLayer.id === "ua_srgk:ca_wasserstand_selten",
			),
		);
